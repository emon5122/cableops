...
export default function TopologyCanvas({
	...
	const [annDrag, setAnnDrag] = useState<{
		...
		startX: number
		startY: number
	}>({ isDragging: false, annId: null, startMouseX: 0, startMouseY: 0, startX: 0, startY: 0 })
	...
	const [annResize, setAnnResize] = useState<{
		...
		startW: number
		startH: number
	}>({ isResizing: false, annId: null, startMouseX: 0, startMouseY: 0, startW: 0, startH: 0 })
	...
	const handleMouseUp = useCallback(() => {
		...
		if (annDrag.isDragging && annDrag.annId) {
			...
			if (pos) {
				onUpdateAnnotation(annDrag.annId, { x: Math.round(pos.x), y: Math.round(pos.y) })
			}
		}
		if (annResize.isResizing && annResize.annId) {
			const sz = liveAnnSizes[annResize.annId]
			if (sz) {
				onUpdateAnnotation(annResize.annId, { width: Math.round(sz.w), height: Math.round(sz.h) })
			}
		}
		setDrag({
			isDragging: false,
		...
		setAnnDrag({ isDragging: false, annId: null, startMouseX: 0, startMouseY: 0, startX: 0, startY: 0 })
		setAnnResize({ isResizing: false, annId: null, startMouseX: 0, startMouseY: 0, startW: 0, startH: 0 })
	}, [drag, livePositions, onDeviceMove, annDrag, liveAnnPositions, annResize, liveAnnSizes, onUpdateAnnotation])
	...
	const [canvasMenu, setCanvasMenu] = useState<{ x: number; y: number; canvasX: number; canvasY: number } | null>(null)
	...
	const getPortConfig = useCallback(
		(deviceId: string, portNumber: number): PortConfigRow | null => {
			return portConfigs.find((pc) => pc.deviceId === deviceId && pc.portNumber === portNumber) ?? null
		},
		[portConfigs],
	)
	...
	return (
		<div
			...
			style={{
				...
				backgroundImage: "radial-gradient(circle, var(--app-canvas-dot) 1px, transparent 1px)",
		...
		>
			...
			{annotations.map((ann) => {
				...
				return (
					...
					>
						...
						>
							<svg viewBox="0 0 10 10" width="14" height="14" className="text-white/30 group-hover:text-white/60">
								<path d="M 8 2 L 2 8 M 8 5 L 5 8 M 8 8 L 8 8" stroke="currentColor" strokeWidth="1" fill="none" />
							</svg>
						</div>
					</div>
				)
					...
					>
						<div
							className={`rounded-xl border-2 shadow-lg transition-shadow ${
								isSelected ? "border-white/50 shadow-white/10" : "border-(--app-border) shadow-black/30"
							}`}
						...
						>
							...
							<div className="p-2 flex flex-wrap gap-1 justify-center">
								{Array.from({ length: device.portCount }, (_, i) => {
									...
									const isSel = selectedPort?.deviceId === device.id && selectedPort.portNumber === pNum
									...
									return (
										<button
											...
											style={{
												...
												color: portTextColor,
												border: connected ? `1px solid ${portColor}88` : "1px solid var(--app-port-border)",
											}}
											onClick={(e) => {
												e.stopPropagation()
											...
											onContextMenu={(e) => {
												e.preventDefault()
												e.stopPropagation()
												setContextMenu({ deviceId: device.id, portNumber: pNum, x: e.clientX, y: e.clientY })
											}}
			...
			>
					...
					return (
						<g key={conn.id}>
							...
							{isWireSel && (
								...
								>
									<circle cx={midX} cy={midY} r={12} fill="#dc2626" opacity={0.9} />
									<text
										x={midX}
										y={midY + 1}
										textAnchor="middle"
				...
				{selectedPort &&
					(() => {
						const pos = getAbsolutePortPos(selectedPort.deviceId, selectedPort.portNumber)
						if (!pos) return null
						const r = PORT_SIZE / 2 + 4
						return (
							<circle cx={pos.x} cy={pos.y} r={r} fill="none" stroke="#ffffff" strokeWidth={2} opacity={0.8}>
								<animate attributeName="r" values={`${r - 2};${r + 2};${r - 2}`} dur="1.5s" repeatCount="indefinite" />
			...
			{devices.length === 0 && (
				<div className="absolute inset-0 flex items-center justify-center" style={{ zIndex: 5 }}>
					<div className="text-center text-(--app-text-muted)">
						...
						<p className="text-sm mt-1">Add a device from the sidebar to get started</p>
					</div>
				</div>
			)}
		</div>
