				...
				<div className="py-1">
					...
					{showIpForwarding && (
						<div className="px-3 py-1.5">
							<button
								...
								onClick={() => onUpdateDevice(device.id, { ipForwarding: !device.ipForwarding })}
							>
								<span className="flex items-center gap-2">
									<ForwardIcon />
									IP Forwarding
								</span>
								<span className={`text-[10px] ${device.ipForwarding ? "text-emerald-400" : "text-(--app-text-muted)"}`}>
					...
					{caps.perPortIp && (
						<div className="px-3 py-1.5 text-[10px] text-(--app-text-dim) italic flex items-center gap-2">
							<NetIcon />
							<span>Right-click each port to set IPs, DHCP, WiFi &amp; NAT</span>
						</div>
					)}
