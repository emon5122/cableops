...
export default function PortContextMenu({
	...
	const [wifiPassValue, setWifiPassValue] = useState(portConfig?.wifiPassword ?? "")
	...
	const saveWifi = useCallback(() => {
		...
		onClose()
	}, [deviceId, portNumber, ssidValue, wifiPassValue, onUpdatePortConfig, onClose])
			...
			<div className="bg-(--app-menu-bg) border border-(--app-border-light) rounded-lg shadow-2xl text-sm min-w-55 overflow-hidden">
				...
				<div className="px-3 py-2 border-b border-(--app-border-light) bg-(--app-surface)">
					...
					{portConfig?.dhcpEnabled && (
						<div className="mt-0.5 text-[10px] text-orange-400">
							DHCP: {portConfig.dhcpRangeStart ?? "?"} – {portConfig.dhcpRangeEnd ?? "?"}
						</div>
					)}
					{portConfig?.ssid && (
				...
				{activePanel === "main" && (
					<div className="py-1">
						...
						{(caps.dhcpCapable || caps.natCapable || caps.wifiHost || caps.perPortIp) && (
						...
						{caps.dhcpCapable && caps.canBeGateway && (
							...
							>
								...
								<span className={`text-[10px] ${portConfig?.dhcpEnabled ? "text-orange-400" : "text-(--app-text-muted)"}`}>
						...
						{caps.natCapable && (
							...
							>
								...
								<span className={`text-[10px] ${portConfig?.natEnabled ? "text-rose-400" : "text-(--app-text-muted)"}`}>
						...
						<div className="px-3 py-2 space-y-2">
							<div className="flex items-center justify-between">
								...
								<button
									...
									onClick={toggleDhcp}
								>
									<div className={`w-3 h-3 rounded-full bg-white shadow transition-transform ${portConfig?.dhcpEnabled ? "translate-x-4" : "translate-x-0.5"}`} />
								</button>
							</div>
							{portConfig?.dhcpEnabled && (
								<>
					...
					<div className="py-1">
						...
						<div className="px-3 py-2 space-y-2">
							<div className="text-[10px] text-teal-400 bg-teal-400/10 rounded px-2 py-1">
								The router IP this interface sends traffic to for destinations outside its subnet.
							</div>
							<label className="text-[10px] text-(--app-text-dim) uppercase tracking-wider block">
								Default Gateway IP
							</label>
