...
export default function WorkspaceSidebar({
	...
	const [editingColorDevice, setEditingColorDevice] = useState<string | null>(
		null,
	);
	const [editingNameDevice, setEditingNameDevice] = useState<string | null>(
		null,
	);
	...
	const handleSubmit = (e: React.FormEvent) => {
		...
		const portNum =
			def === 0 ? 0 : Math.max(1, Math.min(9999, Number(newPorts) || def));
		onAddDevice(newName.trim(), portNum, newColor, newType);
		setNewName("");
		setNewPorts(String(DEVICE_TYPE_DEFAULT_PORTS[newType]));
		setNewColor(
			DEFAULT_COLORS[
				(DEFAULT_COLORS.indexOf(newColor as (typeof DEFAULT_COLORS)[number]) +
					1) %
	...
	return (
		<aside className="w-full shrink-0 border-r border-(--app-border) bg-(--app-surface-alt) overflow-y-auto flex flex-col min-w-0">
			...
			<div className="flex-1 overflow-y-auto px-3 py-2 space-y-1.5">
				...
				{devices.length === 0 && (
					<p className="text-xs text-(--app-text-muted)">No devices yet.</p>
				)}
				{devices
					.filter(
						(d) =>
						...
						return (
							<div
								...
								className={`rounded-lg border transition-colors ${
									isActive
										? "border-white/30 bg-(--app-surface-hover)"
							...
							>
								...
								>
									...
									<span className="text-xs text-(--app-text-muted)">
										{device.portCount > 0 ? `${device.portCount}p · ` : ""}
										{deviceConns.length}c
									</span>

									{/* Port utilization mini bar (only for devices with physical ports) */}
									{device.portCount > 0 && (
										<div
											className="w-10 h-1.5 rounded-full bg-(--app-surface-hover) overflow-hidden shrink-0"
											title={`${wiredConns.length} of ${device.portCount} ports used`}
										>
											<div
												className={`h-full rounded-full transition-all ${wiredConns.length / device.portCount > 0.8 ? "bg-amber-400" : "bg-emerald-500"}`}
												style={{
													width: `${Math.min(100, (wiredConns.length / device.portCount) * 100)}%`,
												}}
											/>
										</div>
									)}
									...
									<div className="relative">
										<button
											...
											onClick={(e) => {
												e.stopPropagation();
												setEditingColorDevice(
													editingColorDevice === device.id ? null : device.id,
												);
											}}
											title="Change color"
										/>
								...
								{isExpanded && (
									<div className="px-3 pb-3 border-t border-(--app-border)">
										...
										{(() => {
											...
											if (!caps) return null;
											const wifiConns = getWifiConnections(
												device.id,
												connections,
											);
											...
											if (caps.wifiClient) {
												const wifiHosts = devices.filter((d) => {
													const dCaps =
														DEVICE_CAPABILITIES[d.deviceType as DeviceType];
													/* Only show hosts that have wifiHost capability AND have an SSID configured on an interface */
													const dSsid = portConfigs.find(
														(pc) => pc.deviceId === d.id && pc.ssid,
													)?.ssid;
													...
													<div className="mt-2 border-t border-(--app-border) pt-2">
														...
														{wifiConns.length > 0 && (
															<div className="space-y-1 mb-1.5">
																{wifiConns.map((wc) => {
																	const hostId =
																		wc.deviceAId === device.id
																			? wc.deviceBId
																			: wc.deviceAId;
																	const hostDev = devices.find(
																		(d) => d.id === hostId,
																	);
																	const wifiIp = portConfigs.find(
																		(pc) =>
																			pc.deviceId === device.id &&
																			pc.portNumber === 0,
																	)?.ipAddress;
																	return (
																		<div
																			key={wc.id}
																			className="flex items-center gap-1 text-xs text-sky-300 flex-wrap"
																		>
																			<Wifi size={9} />
																			<span>{hostDev?.name ?? "?"}</span>
																			{(() => {
																				const hostSsid = portConfigs.find(
																					(pc) =>
																						pc.deviceId === hostId && pc.ssid,
																				)?.ssid;
																				return hostSsid ? (
																					<span className="text-[10px] text-(--app-text-dim) ml-0.5">
																						({hostSsid})
																					</span>
																				) : null;
																			})()}
																			{wifiIp && (
																				<span className="text-[10px] font-mono text-emerald-400 ml-0.5">
																					{wifiIp}
																				</span>
																			)}
																			<button
																...
																{wifiHosts
																	...
																	.map((host) => (
																		...
																		>
																			...
																			<span>{host.name}</span>
																			{(() => {
																				const hSsid = portConfigs.find(
																					(pc) =>
																						pc.deviceId === host.id && pc.ssid,
																				)?.ssid;
																				return hSsid ? (
															...
															{wifiConns.map((wc) => {
																const clientId =
																	wc.deviceAId === device.id
																		? wc.deviceBId
																		: wc.deviceAId;
																const clientDev = devices.find(
																	(d) => d.id === clientId,
																);
																const clientIp = portConfigs.find(
																	(pc) =>
																		pc.deviceId === clientId &&
																		pc.portNumber === 0,
																)?.ipAddress;
																return (
																	<div
			...
			{selectedDevice && !expandedDevice && (
				<div className="p-4 border-t border-(--app-border) bg-(--app-surface)">
					...
					<p className="text-xs text-(--app-text-muted)">
						{DEVICE_TYPE_LABELS[selectedDevice.deviceType as DeviceType] ??
							selectedDevice.deviceType}{" "}
						· {selectedDevice.portCount} ports ·{" "}
						{
							connections.filter(
								(c) =>
									c.deviceAId === selectedDevice.id ||
									c.deviceBId === selectedDevice.id,
							).length
						}{" "}
						connections
					</p>
